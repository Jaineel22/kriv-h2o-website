...
export interface ResidentialProduct {
  ...
  priceNote?: string;
  description: string;
  technologies: { label: string; variant: string }[];
  features: string[];
  storage: string;
  suitableFor: string;
  gradient: string;
...
export default function ProductCard({ product, index = 0 }: ProductCardProps) {
  const waMessage = encodeURIComponent(
    `Hi, I'm interested in the Kriv H2O ${product.name}. Please share more details.`
  );
      ...
      <div className="p-7 flex flex-col gap-5 flex-1">
        ...
        <div className="flex flex-wrap gap-1.5">
          {product.technologies.map((tech) => (
            <FeatureBadge
              key={tech.label}
              label={tech.label}
              variant={tech.variant as BadgeVariant}
            />
          ))}
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 py-3 border-y border-slate-50">
          <div>
            ...
            <p className="text-sm text-slate-700 font-semibold">{product.storage}</p>
          </div>
          <div>
            <p className="text-xs text-slate-400 font-medium mb-0.5">Suitable For</p>
            <p className="text-sm text-slate-700 font-semibold">{product.suitableFor}</p>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-3 mt-auto pt-2">
          ...
          <a
            ...
            className="w-full flex items-center justify-center gap-2 py-3 rounded-xl border border-slate-200 hover:bg-slate-50 text-slate-700 font-semibold text-sm transition-colors duration-200"
          >
